...
data = pd.read_csv("DeepDDI_train.csv")
data = np.array(data)
idx = np.random.permutation(len(data))
train_idx=idx
for i in range(len(train_idx)):
    data[train_idx[i]]
    os.makedirs("DeepDDI_data/train/smiles1/"+str(i))
    os.makedirs("DeepDDI_data/train/smiles2/"+str(i))
    os.makedirs("DeepDDI_data/train/graph1/"+str(i))
    os.makedirs("DeepDDI_data/train/graph2/"+str(i))
    os.makedirs("DeepDDI_data/train/text/"+str(i))

    data1 = mol_to_graph_data_obj_simple(data[train_idx[i]][0])
    torch.save(data1,"DeepDDI_data/train/graph1/"+str(i)+'/graph_data.pt')
    data1 = mol_to_graph_data_obj_simple(data[train_idx[i]][1])
    torch.save(data1,"DeepDDI_data/train/graph2/"+str(i)+'/graph_data.pt')
    
    smiles1 = data[train_idx[i]][0]
    smiles2 = data[train_idx[i]][1]
    file = open("DeepDDI_data/train/smiles1/"+str(i)+"/text.txt","w")
    file.write(smiles1)
    file.close()
    file = open("DeepDDI_data/train/smiles2/"+str(i)+"/text.txt","w")
    file.write(smiles2)
    file.close()
    ...
    text3 = text+'\n'
    file = open("DeepDDI_data/train/text/"+str(i)+"/text.txt","w")
    file.write(text3)
    file.close()
    print(i)